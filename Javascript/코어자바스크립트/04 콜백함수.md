...
객체의 메서드를 콜백 함수로 전달하면 해당 객체를 this로 바라볼 수 없지만, 콜백 함수 내부에서 this가 객체를 바라보게 하려면 별도의 인자로 this를 받는 함수는 원하는 값을 넘기면 되고 그렇지 않은 경우 this의 제어권도 넘겨주게 되므로 사용자가 임의로 원하는 값을 바꿀 수 없다.

그래서 this를 다른 변수에 담아 콜백 함수로 활용할 함수에서 그 변수를 사용하고, 이를 클로저로 만드는 방식을 많이  쓴다.
...
this를 사용하지 않고 같은 결과를 도출하는 예이다. 간결하나 this의 다양한 활용을 못한다. obj1를 다른 객체에 재사용하는 상황의 코드이다.
